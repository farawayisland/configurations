...
## dw | GNU diff with separators, green-colored additions,
...
## fdelce | Recursively find and delete '*-c-executable*' files
...
## gdn | Show changes between consecutive Git commits HEAD~<n+1> and HEAD~<n>
alias gdn='git_diff_with_separators_consecutive_commits'
...
## gdsn | Show changed files between consecutive Git commits HEAD~<n+1> and HEAD~<n>
alias gdsn='git_diff_with_separators_consecutive_commits --stat'
...
## gpull | Fetch from and integrate with another Git repository
## or local Git branch
...
alias gsubls="git config --list | grep --color=auto -oP\
  '(?<=^submodule\.).*?\.git$'"
...
## lmkc | Clean up (remove) all regeneratable files generated by latex
## and bibtex or biber except dvi, postscript, and pdf
...
alias lmklua='latexmk -lualatex -file-line-error -interaction=nonstopmode\
  -synctex=1'

## lmkluas | Recipe for compiling LuaLaTeX documents with latexmk
alias lmkluas='latexmk -lualatex -file-line-error -interaction=nonstopmode\
  -shell-escape -synctex=1'

## lmkpdf | Recipe for compiling pdfLaTeX documents with latexmk
alias lmkpdf='latexmk -pdflatex -file-line-error -interaction=nonstopmode\
  -synctex=1'

## lmkxe | Recipe for compiling XeLaTeX documents with latexmk
alias lmkxe='latexmk -xelatex -file-line-error -interaction=nonstopmode\
  -synctex=1'
...
## git_diff_with_separators_consecutive_commits | Function combination of
## `git diff`
## and `print_separator` for consecutive commits HEAD~<n+1> and HEAD~<n>
autoload -Uz\
  "$ZSH_INTERACTIVE_FUNCTIONS_DIR/git_diff_with_separators_consecutive_commits"
